...
# CSS TÙY BIẾN MENU & GIAO DIỆN (NÂNG TẦM CHUYÊN NGHIỆP)
st.markdown("""
    <style>
    /* Bo góc và làm nổi bật thanh Menu */
...
# Giữ nguyên các thông số kỹ thuật của sếp
...
def main():
    ...
    with st.sidebar:
        ...
        st.subheader("🖥️ LAPTOP MÁY PHA MÀU")
    ...
    # KPI CARDS (Đã được CSS làm nổi bật)
    ...
    m4.metric("📍 VÙNG CAO NHẤT", df_display.groupby('VÙNG')['CP'].sum().idxmax() if not df_display.empty else "N/A")

    st.markdown("<br>", unsafe_allow_html=True)

    # MENU ĐÃ ĐƯỢC THIẾT KẾ LẠI
    tabs = st.tabs(["📊 XU HƯỚNG", "💰 TÀI CHÍNH DEEP", "🩺 SỨC KHỎE MÁY", "📦 KHO LOGISTICS", "🧠 AI ĐỀ XUẤT"])
    ...
    with tabs[1]: # TÀI CHÍNH CHUYÊN SÂU
        ...
        with col_f1:
            st.plotly_chart(px.scatter(deep_df, x="Số_lần_hỏng", y="Tổng_CP", size="Trung_bình_CP", color="LINH_KIỆN", title="TƯƠNG QUAN TẦN SUẤT & CHI PHÍ", color_discrete_sequence=px.colors.sequential.Oranges_r), use_container_width=True)
        with col_f2:
            st.info("**Chiến lược:** Linh kiện phía trên bên phải cần đàm phán lại giá thầu ngay.")
        
        st.plotly_chart(px.treemap(df_display, path=['VÙNG', 'LINH_KIỆN'], values='CP', title="TREEMAP CHI PHÍ (4ORANGES STYLE)", color_discrete_sequence=ORANGE_COLORS), use_container_width=True)
    ...
    with tabs[2]: # SỨC KHỎE MÁY
        ...
        st.write("📋 **DANH SÁCH THIẾT BỊ CẦN CHÚ Ý (HỎNG > 2 LẦN):**")
    ...
    with tabs[3]: # KHO LOGISTICS
        ...
        if wh_data:
            ...
            st.plotly_chart(px.histogram(df_wh, x="VÙNG", color="TRẠNG_THÁI", barmode="group", title="ĐỐI SOÁT KHO LOGISTICS", color_discrete_map={"🟢 ĐÃ TRẢ": "#FF8C00", "🔵 TỒN KHO": "#F39C12", "🟡 ĐANG SỬA": "#D35400"}), use_container_width=True)
